...
if XGBOOST:
    ...
    # Works best with 50.
    clf = RandomForestRegressor(n_estimators=50, n_jobs=2, min_samples_split=5,
...
data = pd.read_csv('../data/proc_data.csv', usecols=to_keep)
data.index = data.id
...
if not LOCAL:
    ...
    test = pd.read_csv('../data/proc_test.csv', usecols=to_keep)
    test.index = test.id
